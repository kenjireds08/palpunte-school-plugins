...
IMPORTANT: 選択肢が2つ以上ある決定を受講生に求めるときは、**平文で聞かず `AskUserQuestion` ツール**で選択肢を提示すること。

- **対象**: 実装方針の選択、ライブラリ選定、設計案の比較、コミット粒度、`/feature-dev` 進行中の設計判断、Plan Mode の計画承認前の分岐など
- **理由**: 非エンジニアの受講生は平文の質問を流し読みして進めてしまうことがある。AskUserQuestion なら UI に選択肢が並び、判断を飛ばされない
- **推奨項目には `(Recommended)` を付ける**（1番目に配置）
- **選択肢は2〜4個**。多すぎても選びにくい
- **乱用しない**: 単純 Yes/No、既に方針が明示されている場面、明らかに1択な場面では不要。テンポが悪くなるので「本当に判断が分かれる場面」だけに絞る
