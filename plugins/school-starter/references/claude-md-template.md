...
### セキュリティチェック系の曖昧発話への対応（YOU MUST）

受講生はセキュリティチェックのコマンドを覚えられない前提で動く。以下の素朴な発話が来たら、**Claude は専門コマンドを叩かせず、意図確認してから `@agent-security-auditor` を自動発火**させる:

**対象となる受講生発話の例**:
- 「セキュリティチェックして」「納品前に見て」「これ安全？」
- 「公開して大丈夫？」「本番デプロイ前チェック」
- 「認証まわり確認して」「RLS 合ってる？」「Supabase のポリシー見て」
- 「脆弱性ないか確認して」「漏洩してない？」

**Claude の応答フロー**:
1. AskUserQuestion で意図確認: 「**納品前のセキュリティチェックですか？**」
   - 選択肢: 「(1) そうです・納品前の全面監査」「(2) 直近の変更部分だけ」「(3) 特定機能だけ（認証・RLS等）」
2. 受講生が返答したら、`@agent-security-auditor` を自然言語発話で呼び出す（例: `@agent-security-auditor 納品前の全面セキュリティ監査をお願いします`）
3. `@agent-security-auditor` 側でスコープを受け取って監査を実施

**NG パターン（絶対にやらない）**:
- 受講生の曖昧発話を無視して Claude 自身でチェックを始める
- 「セキュリティは重要です」などお題目だけで実質的なアクションに繋げない
- 受講生に「`@agent-security-auditor ...` を実行してください」と専門コマンドを覚えさせる
