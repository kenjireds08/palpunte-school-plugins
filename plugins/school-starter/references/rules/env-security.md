...
このプラグイン（school-starter）自身も例外ではない。プラグインを更新（`/plugin` → `Installed` タブ → `school-starter` を選択 → `Update now`）して `/school-starter:setup` を実行した後に、更新時メッセージへ自動表示される「今回の更新で変わったこと」を読んで差分を納得してから使う。
