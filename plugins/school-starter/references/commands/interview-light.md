...
2. 6つのAskUserQuestionで質問:
   - Q1: 用途（Hobby or work?）+ クライアント文脈
   - Q2: エンドユーザー（年齢層・デバイス・利用シーン）
   - Q3: 機能スコープ（MVP / 管理画面 / やらないこと）
   - Q4: お店の基本情報（屋号・営業時間/定休日・主なコース/料金）※モック表示用・テキスト自由回答。屋号は CLAUDE.md/003 にあれば確認
   - Q5: モックアップ範囲（画面数・主要画面・デザイン方向性）
   - Q6: 制約・前提（予算感・納期感・技術希望）
3. `docs/spec-light-[機能名].md` に出力（「お店の基本情報」セクション含む）
