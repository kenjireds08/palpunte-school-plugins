...
次のステップ:
1. クライアント情報・要望を docs/003_client_request.md にまとめる（/interview-light で要件をざっくり固める）
2. デザインの設計図（DESIGN.md）を先に作る（awesome カタログから3案 or 雰囲気の直接指定）
3. モックアップを HTML で作成（クライアント提示用）
4. 見積もりドラフトを作成
5. HTML 御見積書として清書 → ブラウザで PDF 保存してクライアントへ提示

まずは、クライアントから受け取った最初のメッセージや会話内容を
そのまま貼り付けてください。docs/003_client_request.md にまとめます。
（特に追加のメッセージが無ければ、そのまま /interview-light で要件を固めます）
```
