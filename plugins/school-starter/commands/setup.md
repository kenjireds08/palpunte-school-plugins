...
**注意**: Hook は Bash tool だけを対象にしており、**MCP 経由のコマンド実行はブロックできない**。受講生が Claude Code 以外の MCP サーバーを追加するときは、その MCP 固有のツール名で個別に deny する必要がある。Hook は「最後の保険」であって唯一の防御ではない、という位置付けを伝える。
