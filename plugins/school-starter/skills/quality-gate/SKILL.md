...
`.husky/pre-commit` を以下の内容で**作成する**。

> ⚠️ **ここで必ず「許可を求める確認」が出る。事前にユーザーへ予告しておくこと。**
>
> `.husky/` は **Claude Code 本体が「センシティブなファイル」として扱う場所**で、
> 書き込もうとすると次のような確認が出る（2026-08-24 受講生環境の実測で確認）:
>
> ```
> Claude requested permissions to edit ... /.husky/pre-commit which is a sensitive file.
> ```
>
> これは**異常ではなく、正しく働いている安全機能**。git のフックは「コミットのたびに自動実行される
> 場所」なので、AI が黙って書き換えられないようになっている。
>
> **やること**: 書き込む前に、ユーザーにこう伝えてから進める。
> 「次に `.husky/pre-commit` を作ります。**『許可しますか』という確認が出るので、許可してください。**
> ここは *コミットのたびに自動で動く場所* なので、勝手に書き換えられない仕組みになっています。
> 今回は見張りを**増やす**作業なので進めて問題ありません」
>
> **許可が得られない／確認が出ない環境だった場合は、黙って諦めない。** 下の内容を
> コピペ用ブロックで提示し、ユーザー自身に作ってもらうこと（VS Code のファイル一覧で
> プロジェクト直下に `.husky` フォルダ →その中に `pre-commit` という名前・拡張子なしで作成）。
> **この1ファイルが無いと「コミット前に止める」層がまるごと欠ける**ので、
> 作成できたかどうかを必ず確認してから次の STEP へ進む。
...
2. **秘密情報チェックが止めるか**: まず `.husky/pre-commit` が**実在するか**を確認する（作成がスキップされていると、この検証は必ず「素通り」になる）。そのうえで適当なファイルにダミーの秘密をわざと書く → `git add` して `git commit` → pre-commit が ❌ で止めることを確認 → 変更を取り消す（**このコミットは絶対に完了させない**）
   ⚠️ **ダミーの値は、次の1行をそのままコピーして使うこと**（自分で考えて作らない）:

   ```
   const t = "ghp_A1b2C3d4E5f6G7h8I9j0K1l2M3n4O5p6Q7r8";
   ```

   理由は2つあり、どちらも**実測で確認済み**:
   - **AWS 公式ドキュメントの例のキー**（`AKIAIOSFODNN7EXAMPLE` 等）は secretlint が意図的に無視する。使うと素通りする
   - GitHub トークンの検出は **`ghp_` のあとがちょうど36文字のときだけ**成立する。35文字でも37文字でも素通りする（2026-08-29 実測）。自分でランダムな文字列を作ると、まず文字数がずれて「効いていない」と誤解する

   万一これで止まらなかったら、`npx secretlint <そのファイル>` を単体で実行し、`✖ 1 problem` が出るかを先に確認する。単体で検出できているのにコミットが通る場合は、フックが呼ばれていない（`git config core.hooksPath` が `.husky/_` になっているかを見る）
...
> 📎 **PR が実際に届いたときは `dependency-pr` スキルが担当する**（1件ずつの判断・CI 赤の切り分け・コメントの残し方・マージ後の本番確認まで伴走）。ここでは導入時に伝えておく要点だけを扱う。
