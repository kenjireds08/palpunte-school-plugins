...
## YOU MUST: 先に `@agent-security-auditor` を案内する（このスキル実行中は起動しない・案内のみ）
...
**🚨 CSP 投入時の必須実機チェック（v1.16.0〜・T-48 教訓）**:
CSP は **3 段階レビュー（security-auditor / code-reviewer / Codex）の全段階で見逃される** 環境依存セキュリティの代表例。投入直後に**ブラウザコンソールで CSP block エラーが出ていないか必ず目視**する。実例:
- Google Fonts (`fonts.googleapis.com` / `fonts.gstatic.com`) を `style-src` / `font-src` に追加し忘れる → ログイン画面でフォント読み込み失敗 → 「もたつき」発生
- Supabase の `*.supabase.co` を `connect-src` に追加し忘れる → API 呼び出し失敗
- Vercel Analytics / Vercel Insights のドメインを許可し忘れる → 計測されない（本番ログでは silent）

**確認手順**: 本番 URL を開く → DevTools Console を開く → 赤い CSP error が 0 件 / 体感で「もたつき」「白画面」「フォントが標準」が無いことを確認。**ここを飛ばすと納品後にユーザーから「なんか重い」と連絡が来る**。
...
## 🚨 第 4 のレビュー段階「実機検証」（v1.16.0〜・必読）

10 項目チェックを終える前に、**実機検証（本番環境での体感品質確認）** を必ず通す。これは 3 段階レビュー（security-auditor / code-reviewer / Codex）では原理的に検出できない「環境依存バグ」を捕まえる最後の関門。

### なぜ必要か（T-48 / T-62 の実証）

| レビュー段階 | 検出できるもの | 検出できないもの |
|------------|--------------|-----------------|
| ① `@agent-security-auditor`（OWASP 軸） | 認証・RLS・暗号化・入力検証の脆弱性 | フォント読み込み失敗・Vercel ログ・体感品質 |
| ② `code-reviewer`（内部品質軸） | ロジックバグ・命名・重複・エラー処理 | 環境依存・実環境での挙動 |
| ③ Codex（独立第三者軸） | ②で見逃した盲点・触っていないコードの集約検出 | 実環境固有の事象 |
| **④ 実機検証（環境・体感軸）** | **CSP allowlist 漏れ / フォント・DNS / Vercel エラーログ / 体感もたつき** | — |

→ **3 段階レビューでは「設定したつもり」までしか確認できない**。「実際に効いている」「体感品質が崩れていない」は**本番環境で人間（or 並走 Claude）が確認するしかない**。

### 実機検証チェックリスト（必須・5 項目）

**コピペ先**: **本番 URL をブラウザで開く + ターミナルで Vercel CLI**

#### 4-1. 本番 URL で主要フローを 1 周通す
- ログイン / サインアップ / 主要 CRUD / 決済（あれば）/ メール送信（あれば）を**実際に操作**
- 「動いた」だけでなく「**体感もたつきが無い / 白画面が出ない / フォントが標準フォントに化けていない**」を必ず確認

#### 4-2. ブラウザ DevTools Console を必ず開く
- 赤い error が 0 件
- CSP error / CORS error / Mixed Content / 404 が無い
...
#### 4-4. メール送信があれば実機到達確認
- Resend / SendGrid 等を使う場合、**自分のメールに実際に届くか確認**
- スパムフォルダに入っていないか / 送信元名が想定通りか / 本文がレンダリングされているか
- Sandbox 運用（`onboarding@resend.dev`）の場合はその旨をクライアントと合意済みか

#### 4-5. 「環境依存設計」リストの最終目視
以下は**実機でしか確認できない**領域。該当する場合は必ず本番で動作確認:
- CSP / セキュリティヘッダー（特にサードパーティリソース許可漏れ）
- フォント読み込み（Google Fonts / セルフホストフォント）
- DNS / SPF / DKIM（Resend / メール送信ドメイン認証）
- Service Worker / PWA / オフラインキャッシュ
- 環境変数（Production / Preview / Development の値違い）
- Cron / スケジューラ（Vercel Cron は本番でしか発火しない）

### PASS の条件
- 4-1 〜 4-5 すべてで赤い error / 体感品質劣化が 0 件
- 既知の制約（Sandbox メール等）はクライアントと合意済み

### FAIL の対処
1. error の種類を特定（CSP / CORS / 404 / 500 / 環境変数未定義）
2. 該当ファイル / 設定を修正 → 再デプロイ → 4-1 〜 4-5 を**再度全部実施**
3. 「修正して再デプロイ」のループは**必ず本番 URL での再確認まで**を 1 セットとする（「修正コミットした = 完了」と言わない）

### 教訓: 4 段階レビューを 1 つでも飛ばさない
- T-48 で投入した CSP が Google Fonts allowlist 漏れで本番もたつき発生 → 3 段階レビュー全段階で見逃した盲点 → **実機検証（4-2 ブラウザコンソール）で初発見** → 即修正で解決
- 「3 段階レビュー OK = 完璧」ではない。「**3 段階レビュー OK + 実機検証 OK = 完璧**」
...
| # | 項目 | 状態 | メモ |
|---|------|------|------|
| 4-1 | 本番 URL で主要フロー 1 周通す | ✅ / ❌ / ⚠️ | |
| 4-2 | ブラウザ DevTools Console エラー 0 件 | ✅ / ❌ / ⚠️ | 例: CSP / CORS / 404 |
| 4-3 | Vercel Function Logs エラー 0 件 | ✅ / ❌ / ⚠️ | |
| 4-4 | メール実機到達確認（該当時のみ） | ✅ / ❌ / ⚠️ / ➖ | |
| 4-5 | 環境依存設計の最終目視 | ✅ / ❌ / ⚠️ | |

### 判定
🟢 **納品 GO**: 10 項目 + 実機検証 5 項目すべて FAIL / 要確認 0 件
🟡 **要修正後納品**: FAIL あり or 要確認あり → 下記対処後に**実機検証から再実行**
