...
Phase A4.5: 軽量仕様書 → HTML（3点セット完成・商談直前）
  → spec-light（docs/NNN_spec-light-*.md）を、クライアント提示用の要件定義書 HTML に清書する
  → docs/NNN_spec-light_for_client.html を生成（モック・見積書とあわせて「3点セット」が揃う）
  → 「**モックアップ・軽量仕様書・見積書の3点セット**が揃いました。商談に持っていけます」と案内
  → 受講生が見積書だけで止まりそうなら「**仕様書も HTML 清書して3点セットにしますか？**」と提案する
...
Phase B7: 実装開始準備完了
  → まず「品質ゲート（自動チェック）を入れてから始めますか？最初から入れておけば直すものは0件で、
    これから書く全部のコードに効きます」と提案（quality-gate スキル発動・v1.27.0〜）。
    導入済み or 不要の返答なら次へ
  → 「準備完了です！Phase B3 環境構築の最初のタスクから実装を始めましょう」
  → 以降は docs/000_PROJECT_STATUS.md を見ながら、リンク先の詳細を読みつつ実装
  → 各タスクは docs/plans/ 方式で進める（v1.24.0〜）: プラン作成 → docs/plans/<タスクID>-plan.md に保存
    （plansDirectory 設定で自動保存・必要ならリネーム）→ Codex プランレビュー → approve 後に実装
    → review スキルで2段階レビュー → 完了時に docs/plans/archive/ へ移動
    （詳細: ~/.claude/rules/development.md「実装プランは docs/plans/ にファイル化して Codex ゲートを通す」）
```
...
| order_won | phase_b6 | completed | **【v1.27.0〜・品質ゲート提案】**まだ未導入なら「実装に入る前に、品質ゲート（自動チェック）を入れてから始めますか？最初から入れておけば直すものは0件です」と提案（→ quality-gate スキル）。そのうえで「準備完了！実装を開始しましょう。Phase B3 環境構築の最初のタスクから進めます」 |
