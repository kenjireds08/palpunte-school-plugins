...
### 🟠 N1. `/new-project` の .env 案内が「相談段階」モードで省略されるブレ

- **発見元**: 2026-06-01 ちーけん lesson-02 dry-run（隔離環境 personal-gym-reservation）
- **問題**: `/new-project` の Step 4-4 に .env.local / .env.example 作成の OS別案内（v1.21.5 で OS別化済み）が定義されているが、「相談段階」を選んだ実機で Step 4-4 がスキップされ、Step 5 で Claude が独自解釈「.envなどは受注後の本実装フェーズで扱います（今は不要）」を出力。Step 4-4 が必ず実行されるかブレがある
- **暫定対応**: lesson-02 コピペシートのステップ2末尾に `.env.local と .env.example をターミナルで作る…` プロンプトが既に存在 → 案内されなかった受講生はコピペシートで補完できる（運用でカバー）
- **恒久修正案（v1.21.8）**:
  1. new-project.md Step 4-4 冒頭に「**YOU MUST: 案件状態（受注確定/相談段階/自分用）に関わらず必ず実行**」を明示
  2. Step 5 の「相談段階」分岐冒頭に「.env はすでに Step 4-4 で作成済みの前提」を1行追加
- **重要度**: 🟠（次リリース推奨・運用でカバーできているので緊急ではない）
...
- **対応案（2026-05-24 ちーけん指摘で優先順位を更新）**:
  - (A) **【最優先】Hook で停止＆自由入力待ち**: `DESIGN.md` を Write した直後の PostToolUse で「モックはまだ作るな。ユーザーのモック作成プロンプト入力を待て」を inject。**自然に停止して自由入力待ちになる → コピペシートの長文モックプロンプトをそのまま貼れる**。Claude のセリフは「DESIGN.md 完成。次はモックです。どんな構成がいいですか？」程度
  - (B) ~~AskUserQuestion で強制ゲート~~ → **却下寄り**: モック着手前に AskUserQuestion（選択式）が出ると**コピペシートの長文モックプロンプトを貼り付けづらい**（ちーけん 2026-05-24）。停止はするが受講生の貼り付け導線と相性が悪い
  - (C) コピペシート運用の徹底（`lesson-02.html` の DESIGN.md プロンプトに「モックはまだ作らないで」明記）は (A) の補助として継続
